...
def admin_delete_task():
    ...
    # ✅ Soft delete — active: False করে দিন
    result = db_mongo["tasks"].update_one(
        {"task_id": task_id},
        {"$set": {"active": False}}
    )
...
def live_candle():
    ...
    try:
        # ✅ candles (1M) — update_candle যেখানে সেভ করে
        last_candle = db_mongo["candles"].find_one(
            {},
            sort=[("time", -1)]
        )
...
def execute_trade():
    ...
    # ফ্রন্টএন্ডের action/amount ফরম্যাট সাপোর্ট
...
def admin_clear_field():
    ...
    # ✅ হোয়াইটলিস্ট — শুধু এই ফিল্ডগুলো ক্লিয়ার করা যাবে
    allowed_fields = [
        "banner_ad_code",
        "task_banner_ad",
        "task_popup_ad",
    ...
    # ✅ $set দিয়ে খালি স্ট্রিং (ফিল্ড থাকবে, ভ্যালু খালি)
    admin_config_col.update_one(
        {"_id": "global"},
        {"$set": {field_name: ""}}
    )
...
# ================= এডমিন টাস্ক API (MongoDB Only) =================

@app.route("/api/admin/tasks/list", methods=["GET"])
def admin_tasks_list():
    """এডমিন প্যানেলের জন্য টাস্ক লিস্ট (MongoDB)"""
    if not session.get("admin_logged_in"):
        return jsonify({"error": "Unauthorized"}), 401
    
    tasks = list(db_mongo["tasks"].find({}, {"_id": 0}))
    for t in tasks:
        t["_id"] = t.get("task_id")  # ফ্রন্টএন্ড _id চায়
...
@app.route("/api/admin/tasks/toggle/<task_id>", methods=["POST"])
def admin_tasks_toggle(task_id):
    """টাস্ক সক্রিয়/নিষ্ক্রিয় টগল"""
    if not session.get("admin_logged_in"):
        return jsonify({"error": "Unauthorized"}), 401
...
@app.route("/api/admin/tasks/delete/<task_id>", methods=["DELETE"])
def admin_tasks_delete(task_id):
    """টাস্ক ডিলিট (সফট ডিলিট - active false করে)"""
    if not session.get("admin_logged_in"):
        return jsonify({"error": "Unauthorized"}), 401
    ...
    if result.modified_count > 0:
        return jsonify({"success": True, "message": "টাস্ক ডিলিট হয়েছে!"})
    else:
        return jsonify({"success": False, "message": "Task not found"}), 404
...
def user_check_stat():
    ...
    async def check():
        ...
        try:
            ...
            if post_id:
                ...
                # গ্রুপ হলে মেম্বার লিস্ট (যদি গ্রুপ হয়)
                if not hasattr(entity, 'broadcast'):  # গ্রুপ চেক
                    try:
    ...
    # ✅ অ্যাসিঙ্ক ফাংশন রান করান
    try:
        data = run_async(check())
        
        # ✅ শুধুমাত্র সফল রিকোয়েস্ট ক্যাশে করুন
        if data.get("success"):
